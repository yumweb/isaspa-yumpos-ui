...
const MyCalendar = (props) => {
    ...
    if (props.appointmentData.length > 0) {
      ...
      for (let data of props.appointmentData) {
        const [year, month, day] = data.appointmentTime
          .split("T")[0]
          .split("-");
        const [hour, minute] = data.appointmentTime.split("T")[1].split(":");
        ...
        formattedTime = moment(utcTime).format("hh:mm A");

        const title = `${formattedTime} - ${data.sale.customer.firstName} ${data.sale.customer.lastName}`;
  ...
  const eventStyleGetter = (event, start, end, isSelected, time) => {
      ...
      {props.loading ? (
        ...
        <div className="px-2 py-2">
          {props.appointmentData.length > 0 ? (
            ...
            <Calendar localizer={mLocalizer} style={{ height: 500 }} />
          )}
        </div>
      )}
    </>
